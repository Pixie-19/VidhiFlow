...
app.get('/sse', async (req, res) => {
  ...
  // Store transport by session ID so POST /messages can find it
  // Note: sessionId is available immediately on the instance
  transports.set(transport.sessionId, transport);

  server.server.onclose = () => {
    console.log('Server closed, cleaning up transport');
...
app.listen(PORT, () => {
  console.log(`VidhiFlow MCP server running on port ${PORT}`);
